...
pub fn start() void {
    ...
    const extensions = supermd.cmark_list_syntax_extensions(
        supermd.c.cmark_get_arena_mem_allocator(),
    );

    if (builtin.single_threaded) {
        cmark = supermd.Ast.CmarkParser.default(extensions);
        return;
    }
    ...
    for (0..thread_count) |idx| {
        const _cmark = supermd.Ast.CmarkParser.default(extensions);
...
fn renderPage(
    ...
    defer p.end();
    // context.pageFind = struct {
    //     const empty: context.Value = .{
    //         .array = .{
    //             .len = 0,
    //             .empty = true,
    //             ._items = &.{},
    ...
    //     };
    //     fn func(
    //         search: context.PageSearchStrategy,
    //     ) error{OutOfMemory}!context.Value {
    //         switch (search) {
    //             else => {
    //                 return context.Value.Optional.Null;
    //             },
    //             .subpages => {
    //                 return empty;
    //             },
