...
const TemplateContext = template.TemplateContext;
...
pub const SuperVM = struct {
    ...
    pub const Template = SuperTemplate(OutWriter);
    ...
    pub fn init(
        ...
        return .{
            ...
            .script_ctx = TemplateContext.init(context),
            .out = out_writer,
            .err = err_writer,
            .state = .{
                .init = .{
    ...
    pub fn wantedTemplateName(self: SuperVM) []const u8 {
    ...
    pub fn insertTemplate(self: *SuperVM, path: []const u8, html: []const u8) void {
    ...
    pub fn setQuota(self: *SuperVM, q: usize) void {
        self.quota = q;
    }
    ...
    pub fn run(self: *SuperVM) Exception!void {
    ...
    fn runInternal(self: *SuperVM) Exception!void {
        ...
        while (self.quota > 0) : (self.quota -= 1) {
            ...
            const continuation = t.eval(&self.script_ctx, self.out, self.err) catch |err| switch (err) {
                error.OutOfMemory,
                error.OutIO,
                error.ErrIO,
                => |e| return e,
    ...
    fn discoverTemplates(self: *SuperVM) DiscoverException!void {
    ...
    fn loadTemplate(self: *SuperVM) !void {
