...
pub fn website(project: *std.Build, options: Options) void {
    const run_zine = project.addSystemCommand(&.{ "zine", "release" });
    for (options.build_assets) |a| {
    ...
    project.getInstallStep().dependOn(&run_zine.step);
}
