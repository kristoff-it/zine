...
/// You can use zine.assets() for convenience.
pub const BuildAssetMap = std.StringHashMapUnmanaged(BuildAsset);
...
/// The returned value can be modified before being passed to `Site`
/// or `MultilingualSite`.
pub fn assets(b: *std.Build, values: []const BuildAsset) BuildAssetMap {
    var map: BuildAssetMap = .{};

    for (values) |v| {
        map.putNoClobber(b.allocator, v.name, v) catch unreachable;
    }
...
pub const Site = struct {
    ...
    host_url: []const u8,
    /// Set this value if your website is hosted
    /// under a subpath of `host_url`.
    ///
    /// `foo.com/bar` => `.url_path_prefix = "bar"`
    url_path_prefix: []const u8 = "",
    /// If you want your site to be placed inside
    /// of a subdirectory of the output directory.
    /// Zig build's output directory is `zig-out`
    /// by default, customizable by passing `-p` to
    /// the build invocation.
    output_path_prefix: []const u8 = "",
    layouts_dir_path: []const u8,
    content_dir_path: []const u8,
    assets_dir_path: []const u8,
    /// Subpaths in `assets_dir_path` that will be
    /// installed unconditionally. All other assets
    /// will be installed only if referenced by a
    /// content file or a layout by using `$site.asset()`.
    static_assets: []const []const u8 = &.{},
    /// Buld-time assets, use `zine.assets()` for
    /// a convenient way of initializing this value.
    build_assets: BuildAssetMap = .{},
...
pub const MultilingualSite = struct {
    ...
    /// It must not contain a path other than `/`.
    host_url: []const u8,
    /// Directory that contains one Ziggy file
    /// per localized variant of this website.
