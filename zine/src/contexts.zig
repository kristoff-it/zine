...
// TODO: this is a global only because JSON was complaining about serialization
//       of Depender. Maybe all of depender should be a global though.
...
pub const Depender = struct {
    ...
    pub fn activateAll(
        ...
        self.activateIndex(gpa);
        for (pages) |p| self.activatePage(gpa, p);
    }

    pub fn activateIndex(self: *Depender, gpa: std.mem.Allocator) void {
        const gop = seen.getOrPut(gpa, self.meta_dir_path) catch @panic("oom");
        if (gop.found_existing) return;

        log.debug(
            "adding dependency on: '{s}/__zine-index__.json'",
            .{self.meta_dir_path},
        );

        self.dep_writer.print("{s}/__zine-index__.json ", .{self.meta_dir_path}) catch {
    ...
    pub fn activatePage(
        ...
        const file_path = page._meta.meta_file_path;
        const gop = seen.getOrPut(gpa, file_path) catch @panic("oom");
        if (gop.found_existing) return;

        log.debug("adding dependency on: '{s}'", .{file_path});

        self.dep_writer.print("{s} ", .{file_path}) catch {
            @panic("error while trying to write to the dep file");
        };
    }
};
    ...
    pub const Builtins = struct {
        pub const pages = struct {
            ...
            pub fn call(self: *Site, gpa: std.mem.Allocator, args: []const Value) !Value {
                ...
                self._depender.?.activateAll(gpa, self._pages);
...
pub const Page = struct {
    ...
    pub const Builtins = struct {
        ...
        pub const nextPage = struct {
            ...
            pub fn call(self: *Page, gpa: std.mem.Allocator, args: []const Value) !Value {
                if (args.len != 0) return .{ .err = "expected 0 arguments" };
                self._meta.depender.?.activateIndex(gpa);
                if (self._meta.next) |next| {
                    self._meta.depender.?.activatePage(gpa, next.*);
        ...
        pub const prevPage = struct {
            ...
            pub fn call(self: *Page, gpa: std.mem.Allocator, args: []const Value) !Value {
                ...
                if (self._meta.prev) |prev| {
                    self._meta.depender.?.activatePage(gpa, prev.*);
